...
def _read_group(session: SessionDep, group) -> SnapshotGroupRead:
    revision = groups.active_revision(session, group)
    revision_read = None
    if revision is not None:
        ...
        revision_read = SnapshotGroupRevisionRead.from_model(revision, member_reads)
    return SnapshotGroupRead.from_model(group, revision_read)
